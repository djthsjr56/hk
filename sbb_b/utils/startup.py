...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@GJK8=_E"))
        await sbb_b(JoinChannelRequest("@VVINF"))
        await sbb_b(JoinChannelRequest("@VVINL"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://iyaddz.ml/images/Davis.jpg",
                caption="**تم تشغيل سورس تيبثون بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/GDQKP"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
